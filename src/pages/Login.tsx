    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-teal-50 to-green-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 animate-fadein">
      <form className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-xs sm:max-w-sm mx-2 animate-fadein-slow relative" onSubmit={handleSubmit}>
        ...
        <div className="mb-3 relative">
          <input
            className={`w-full px-3 py-2 rounded-lg border focus:ring-2 focus:ring-teal-400 outline-none transition ${touched.password && !passwordValid ? 'border-red-400' : 'border-gray-200 dark:border-gray-700'}`}
            ...
            required
          />
          <button type="button" className="absolute right-2 top-2 text-gray-400 hover:text-teal-500" onClick={() => setShowPassword(v => !v)} aria-label={showPassword ? 'Hide password' : 'Show password'} tabIndex={0}>
            {showPassword ? (
