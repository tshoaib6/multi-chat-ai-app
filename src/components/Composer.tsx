...
export default function Composer({ onSaveChat }: { onSaveChat?: (title: string, messages: any[]) => void }) {
	const [val, setVal] = useState('');
	const { currentTopicId, transcript, addEntry } = useApp();

	function send() {
		const text = val.trim();
		if (!text) return;
		// If this is the first message, trigger chat save
		if (transcript.length === 0 && onSaveChat) {
			const entry = makeEntry(currentTopicId, 'user', text);
			onSaveChat('Untitled', [entry]);
			addEntry(entry);
		} else {
